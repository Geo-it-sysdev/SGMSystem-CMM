...
public function fetch_students() {
    ...
    $section = $this->input->get('section'); // optional
...
//    end add / edit / delete student
  ...
  public function get_students_by_section()
  {
      ...
      $user_id = $this->session->userdata('po_user'); // logged-in user
  
      // Get already graded students for this activity
      $graded = $this->db->select('user_id')
          ->get_where('tbl_activities_lines', ['activities_id_header' => $activity_type_id])
          ->result_array();
  
      $graded_ids = array_column($graded, 'user_id');
  
      // Select students in the section, belonging to logged-in user, and not yet graded
      $this->db->select('id, fullname, section, gender')
               ->where('section', $section)
               ->where('user_id', $user_id); // filter by logged-in user
...
public function save_grade_bulk()
{
    ...
    $section = $this->input->post('section');
    $scores = $this->input->post('scores'); // array
    ...
    // Get the overall score limit for this activity
    $activity = $this->db->get_where('tbl_activities_header', ['id' => $activity_type_id])->row();
    if (!$activity) {
    ...
    foreach ($scores as $student_id => $score) {
        ...
        // Check if score exceeds overall
        if ($score > $overall) {
            echo json_encode([
                'status' => 'error',
        ...
        // prevent duplicates
        $exist = $this->db->get_where('tbl_activities_lines', [
        ...
        // get student info
        $student = $this->db->get_where('tbl_students', ['id' => $student_id])->row();
        if (!$student) continue;
...
public function fetch_activities() {
    $grade_level = $this->input->post('grade_level'); // optional filter
    $user_id    = $this->session->userdata('po_user'); // logged-in user
    $user_type  = $this->session->userdata('user_type'); // get user type
    ...
    // Apply user filter only if the user is NOT Principal, Registrar, or Guidance Councilor
...
public function fetch_final_grades() {
    ...
    // Determine weights based on grade level
...
public function upload_profile_photo()
{
    ...
    // Delete old image (only if it's not the dummy default)
...
public function fetch_activity_by_grade() {
    $grade_level = $this->input->post('grade_level');
    $user_id     = $this->session->userdata('po_user');
    $user_type   = $this->session->userdata('user_type'); // get user role
    ...
    // Only apply user_id filter if the user is NOT Principal, Registrar, or Guidance Councilor
    if (!in_array($user_type, ['Principal', 'Registrar', 'Guidance Councilor'])) {
        $this->db->where('a.user_id', $user_id);
    }
...
public function fetch_students_by_section() {
    $section   = $this->input->post('section');
    $user_id   = $this->session->userdata('po_user');     // logged-in user
    $user_type = $this->session->userdata('user_type');   // user role
    ...
    // Apply user_id filter only if user is NOT Principal, Registrar, or Guidance Councilor
    if (!in_array($user_type, ['Principal', 'Registrar', 'Guidance Councilor'])) {
        $this->db->where('a.user_id', $user_id);
    }
