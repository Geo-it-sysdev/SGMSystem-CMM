    ...
    ?>
                                    ...
                                    <div class="col-md-4 text-center">
                                        <div class="profile-user position-relative d-inline-block mb-4">
                                            ...
                                            <img id="profile-preview"
                                                src="<?= base_url('assets/img/user-dummy-img.jpg') ?>"
            ...
            $(document).ready(function() {
                            ...
                            <?php if ($user_type === 'Teacher'): ?> {
                                ...
                                render: function(data) {
                                    ...
                                    if (['Principal', 'Guidance Counselor', 'Registrar',
                                        ...
                                        currentUser) {
                                        btns +=
                                            `<button class="btn btn-sm btn-outline-primary editBtn" data-id="${data.id}"><i class="bx bx-edit me-2"></i>Edit</button>`;
                                        btns +=
                                            `<button class="btn btn-sm btn-outline-danger deleteBtn" data-id="${data.id}"><i class="bx bx-trash me-2"></i>Delete</button>`;
                                    }
                                    ...
                                    btns +=
                                        `<button class="btn btn-sm ${statusClass} toggleStatusBtn" data-id="${data.id}" data-status="${data.status}"><i class="bx ${statusIcon} me-2"></i>${statusText}</button>`;
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (res.status === 'success') {
                                ...
                                let alertClass = newStatus === 'inactive' ?
                                    'alert-success' :
                                    'alert-secondary';
                ...
                `);
                                ...
                                let gradeLevel = tabPane.find('h5').text().replace(
                                    ' Students',
                                    '').trim();
                ...
                $('.tab-pane').on('click', '.editBtn', function() {
                    let id = $(this).data('id');
                    $.getJSON("<?= site_url('StudentController/edit_student/'); ?>" + id, function(
                    data) {
                        if (data.error) {
                ...
                $('#studentForm').on('submit', function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            } else if (res.status === 'success' || res.status ===
                                'updated') {
                ...
                $('.tab-pane').on('click', '.deleteBtn', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.getJSON("<?= site_url('StudentController/delete_student/'); ?>" +
                                id,
                                function(res) {
